    ...
    c.keys match {
      case Some(keys) if keys.nonEmpty =>
        ...
        NodeTag.withName(key) match {
          ...
          case NodeTag.T_CreatedbStmt   => value.as[CreatedbStmt]
          case NodeTag.T_DropdbStmt     => value.as[DropdbStmt]
          case NodeTag.T_CreateStmt     => value.as[CreateStmt]
          case NodeTag.T_ColumnDef      => value.as[ColumnDef]
          case NodeTag.T_TypeName       => value.as[TypeName]
          case NodeTag.T_DropStmt       => value.as[DropStmt]
          case NodeTag.T_TruncateStmt   => value.as[TruncateStmt]
          case NodeTag.T_AlterTableStmt => value.as[AlterTableStmt]
          case NodeTag.T_AlterTableCmd  => value.as[AlterTableCmd]
          case _ =>
